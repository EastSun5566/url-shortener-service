...
因為 UUID/Snowflake 這類產生全域唯一 ID 的演算法因產生字串長度過長所以不考慮，我最後選擇在 Redis 中建立一個全域 counter，每次產生 key 時都會先從 Redis 中取得 count，然後用 [INC](https://redis.io/commands/incr) 命令將 count +1，再把 count 編碼成 base62，這樣就可以確保 key 全域唯一，並且無需在寫入 DB 前再進行唯一性檢查。
